...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Firestoreでエンジニア情報を検索
      const q = query(collection(firestore, 'engineers'), where('name', '==', name), where('location', '==', location))
      const querySnapshot = await getDocs(q)

      if (!querySnapshot.empty) {
        // ユーザー情報を context に保存
        setUser({ name, location, type: 'engineer' })
      ...
      if (error instanceof Error) {
        setErrorMessage(`ログイン処理中にエラーが発生しました: ${error.message}`)
      } else {
        setErrorMessage('ログイン処理中に予期しないエラーが発生しました。')
      }
    ...
    <Layout type='engineer'>
      <div className="max-w-md mx-auto bg-white p-8 rounded-lg shadow-md">
        ...
        <div className="mt-4 text-center">
          <Link href="/engineer/register" className="text-blue-600">新規登録はこちらから</Link>
        </div>
      </div>
    </Layout>
  )
